...
class PriceRepository {
    ...
    suspend fun getPriceWithPremise(premiseId: String): DataOrException<List<ItemPrice>, Exception> {
        val dataOrException = DataOrException<List<ItemPrice>, Exception>()
        try {
            dataOrException.data =
                Firestore.ColRefFilter(collectionName, "premise", premiseId).get().await()
                    .map { document ->
                        document.toObject(ItemPrice::class.java)
                    }
    ...
    suspend fun getPriceWithItem(itemId: String): DataOrException<List<ItemPrice>, Exception> {
        val dataOrException = DataOrException<List<ItemPrice>, Exception>()
        try {
            dataOrException.data =
    ...
    suspend fun getPriceWithPremiseAndItem(
        premiseId: String,
        itemId: String
    ): DataOrException<List<ItemPrice>, Exception> {
        val dataOrException = DataOrException<List<ItemPrice>, Exception>()
        try {
            dataOrException.data =
