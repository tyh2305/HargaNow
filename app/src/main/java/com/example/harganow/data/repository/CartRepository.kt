...
class CartRepository {
    ...
    suspend fun addToCart(premiseId: String, item: CartItem) {
        ...
        DocRef(collectionName, currentUser.uid + premiseId).update(
            "items",
    ...
    suspend fun removeFromCart(premiseId: String, items: MutableList<Map<ItemPrice, Int>>) {
        if (currentUser?.uid == null) {
            throw Exception("User not logged in")
        }
        val cart = getCart(premiseId)
        ...
        val newItems = cart.data!!.items?.filter { item ->
            !items.any { it.keys.first().item.id == item.id.toString() }
        }
